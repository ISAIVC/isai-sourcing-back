...
- **2026-09-18** — Feature livrée : flag "tier-1 investor" (`tier_1_investor` bool + `tier_1_investors_matched` text[] sur `sourcing_mv`/`sourcing_view`/`search_companies`, matché depuis `bcv.all_investors` vs `vc_funds` où `tier=1`, case/whitespace-insensitive), preset toolbar "Tier 1 backed". Presets passés en mode merge/stack (`applyPreset` dans `HomePage.jsx`) au lieu de full-replace, pour pouvoir en cumuler plusieurs à la suite. Migration `isai-sourcing-back/supabase/migrations/tier1_investor_flag.sql` pas encore appliquée en base — ne PAS la lancer via le SQL Editor/MCP : la seule reconstruction de `sourcing_mv` prend ~4min (mesuré), au-delà des deux timeouts (2min éditeur, ~60-100s proxy MCP) ; lancer via psql direct avec `statement_timeout` étendu.
